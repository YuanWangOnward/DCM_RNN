...
class Subject:
    ...
    def __init__(self, flags=None, t_delta=None, n_region=None, n_time_point=None, n_stimuli=None):
        ...
        self.Wxx = np.array([[-1, 0, 0],
                             [0.8, -1, 0.4],
                             [0.4, 0.8, -1]], dtype=np.float32) * t_delta + np.eye(n_region, n_region, 0,
                                                                                   dtype=np.float32)
        ...
        if self.flags.random_hemodynamic_parameter:
            ...
            self.hemodynamic_parameters_deviation_normalized = pd.DataFrame(
                np.zeros(h_shape[0] * h_shape[1]).reshape(h_shape[0], h_shape[1]), \
    ...
    def sample_hemodynamic_parameters(self, hemodynamic_parameters_mean=None, hemodynamic_parameters_variance=None,
                                      deviation_constraint=0.4):
