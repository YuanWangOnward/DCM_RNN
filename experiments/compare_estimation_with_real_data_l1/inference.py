...
def calculate_loss(du_hat, y_true, y_noise, loss_weighting, hemodynamic_parameter_mean=None):
    ...
    loss_y = loss_weighting['y'] * 0.5 * np.sum(
        [np.sum(y_e[:, r] ** 2 * np.exp(y_noise[r])) for r in range(du_hat.get('n_node'))])

    # loss q
    loss_q = loss_weighting['q'] * 0.5 * du_hat.get('n_time_point') * np.sum(
        [-y_noise[r] for r in range(du_hat.get('n_node'))])
...
for epoch in range(MAX_EPOCHS):
    ...
    loss_original = calculate_loss(du_hat, y_target, y_noise, loss_weighting, np.array(du.get('hemodynamic_parameter')))
    ...
    try:
        ...
        y_noise_index = du_hat.variable_names_in_graph.index('y_noise')
        y_noise = grads_and_vars[y_noise_index][1] - grads_and_vars[y_noise_index][0] * step_size
        loss_updated = calculate_loss(du_hat, y_target, y_noise, loss_weighting,
                                      np.array(du.get('hemodynamic_parameter')))
    except:
    ...
    y_noise = solve_for_noise_variance(du_hat, y_target, y_noise, loss_weighting)
