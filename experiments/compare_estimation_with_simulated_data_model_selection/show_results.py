...
for hypo in [0, 1]:
    for sub in [0, 1, 2, 3, 4]:
        ...
        'hypothesis': {'value': hypo, 'short_name': 'hypo'},
        'subject': {'value': sub, 'short_name': 'sub'}
        }
        if SETTINGS['subject']['value'] == 0:
            SETTINGS['if_random_h_parameter']['value'] = 0
        else:
            SETTINGS['if_random_h_parameter']['value'] = 1
        ...
        CORE_PATH = os.path.join(DATA_PATH, 'core_' + READ_NAME_EXTENTION + '.pkl')
        ...
        IF_SHOW_CROSS_SUBJECT_RESULTS = False
...
if IF_SHOW_CROSS_SUBJECT_RESULTS:
