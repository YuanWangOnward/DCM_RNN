...
for hypo = [0, 1]
    for sub = [0 1 2 3 4]
        ...
        for i =1:length(keys)
            key = keys{i};
            if ~strcmp(key, 'hypothesis')
                temp{end + 1} = [num2str(SETTINGS.(key).short_name), '_', num2str(SETTINGS.(key).value)];
            end
        end
        READ_NAME_EXTENTION = lower([sprintf('%s_',temp{1:end-1}),temp{end}]);
        ...
        READ_PATH = fullfile(DATA_PATH, ['DCM_initial_', READ_NAME_EXTENTION, '.mat']);
        ...
        %% specify supports of the connectivity matrices
        if SETTINGS.hypothesis.value == 0
            % need to do nothing
        elseif SETTINGS.hypothesis.value == 1
            temp = zeros(n_node, n_node);
            temp(2,1) = 1;
            DCM_corrected.b(:,:,3) = temp;
        else
            error('hyphthesis value is  with specified SNR is not found')
        end
