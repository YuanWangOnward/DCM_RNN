...
'if_random_h_parameter': {'value': 0, 'short_name': 'rh'},
'snr': {'value': 3, 'short_name': 'snr'},
'hypothesis': {'value': 0, 'short_name': 'hypo'},
'subject': {'value': 0, 'short_name': 'sub'}
}
...
READ_NAME_EXTENTION = '_'.join([SETTINGS[k]['short_name'] + '_' + str(SETTINGS[k]['value'])
                                for k in keys if k not in ['hypothesis']])
...
DATA_PATH = os.path.join(EXPERIMENT_PATH, 'data', 'du_DCM_RNN_' + READ_NAME_EXTENTION + '.pkl')
...
if SETTINGS['hypothesis']['value'] == 0:
    mask = du.create_support_mask()
elif SETTINGS['hypothesis']['value'] == 1:
    mask = du.create_support_mask()
    mask['Wxxu'][2] = np.zeros(mask['Wxx'].shape)
    mask['Wxxu'][2][1, 0] = 1
else:
    ValueError('hypothesis value not understood')
...
dcm_rnn_free_energy['decay'] = np.log(np.array(du_hat.get('hemodynamic_parameter')['k']) / 0.64)
dcm_rnn_free_energy['epsilon'] = np.log(np.array(du_hat.get('hemodynamic_parameter')['epsilon']) / 1.)
dcm_rnn_free_energy['Ce'] = np.array(np.exp(-du_hat.y_noise))  # save as spm does, however it seems a bug
scipy.io.savemat(SAVE_PATH_FREE_ENERGY, mdict={'dcm_rnn_free_energy': dcm_rnn_free_energy})
...
for n_setting in [0, 1, 2, 3, 4]:
    ...
    if n_setting == 0:
        SETTINGS = {
            ...
            'if_random_h_parameter': {'value': 0, 'short_name': 'rh'},
            'snr': {'value': 3, 'short_name': 'snr'},
            'hypothesis': {'value': HYPOTHESIS, 'short_name': 'hypo'},
            'subject': {'value': 0, 'short_name': 'sub'}
        }
    else:
        SETTINGS = {
            ...
            'snr': {'value': 3, 'short_name': 'snr'},
            'hypothesis': {'value': HYPOTHESIS, 'short_name': 'hypo'},
            'subject': {'value': n_setting, 'short_name': 'sub'}
        }
