...
def calculate_loss(du_hat, y_true, y_noise, loss_weighting, hemodynamic_parameter_mean=None):
    # loss y
    y_e = du_hat.get('y') - y_true
    loss_y = loss_weighting['y'] * 0.5 * np.sum([np.sum(y_e[:, r] ** 2 * np.exp(y_noise[r])) for r in range(du_hat.get('n_node'))])

    # loss q
    loss_q = loss_weighting['q'] * 0.5 * du_hat.get('n_time_point') * np.sum([-y_noise[r] for r in range(du_hat.get('n_node'))])
    ...
    loss_c = loss_weighting['prior_Wxu'] * np.sum(np.abs(du_hat.get('C')))
    ...
    loss_prior_h = loss_weighting['prior_h'] * 0.5 * np.sum(np.square(para_h - mean_h)) * 256

    # loss prior hyper
    loss_prior_hyper = loss_weighting['prior_hyper'] * 0.5 * np.sum(np.square(y_noise - 6)) * 128
...
def solve_for_noise_variance(du_hat, y_true, y_noise, loss_weighting):
    y_e = du_hat.get('y') - y_true
    output = np.zeros(du_hat.get('n_node'))
    for r in range(du_hat.get('n_node')):
        a = loss_weighting['y'] * 0.5 * np.sum(y_e[:, r] ** 2)
        b = loss_weighting['q'] * 0.5 * du_hat.get('n_time_point')
        temp = y_noise[r]
        for s in range(512 * 8):
            l_original = a * np.exp(temp) - b * temp + loss_weighting['prior_hyper'] * 0.5 * 128 * (temp - 6) ** 2
            d_temp = a * np.exp(temp) - b + loss_weighting['prior_hyper'] * 128 * (temp - 6)
            temp = temp - 1 / 512. * np.sign(d_temp)
            l_updated = a * np.exp(temp) - b * temp + loss_weighting['prior_hyper'] * 0.5 * 128 * (temp - 6) ** 2
...
SETTINGS['h1_s0_n1'] = {'if_update_h_parameter': True,
                        'if_extended_support': False,
                        'if_noised_y': True}
SETTINGS['h1_s2_n1'] = {'if_update_h_parameter': True,
                        'if_extended_support': True,
                        'if_extended_support_b': True,
                        'if_noised_y': True}
...
x_parameter_initial['C'][0, 0] = 0.
x_parameter_initial['C'][1, 1] = 0.
...
if 'if_extended_support_b' in SETTINGS[CONDITION] and SETTINGS[CONDITION]['if_extended_support_b']:
    loss_weighting['prior_Wxxu'] = 8.
...
if 'if_extended_support_b' in SETTINGS[CONDITION] and SETTINGS[CONDITION]['if_extended_support_b']:
    mask['Wxxu'][2] = np.ones((du.get('n_node'), du.get('n_node')))
...
isess.run(tf.assign(dr.loss_y_weight, loss_weighting['y']))
isess.run(tf.assign(dr.loss_q_weight, loss_weighting['q']))
isess.run(tf.assign(dr.loss_prior_x_weight, loss_weighting['prior_x']))
isess.run(tf.assign(dr.loss_prior_h_weight, loss_weighting['prior_h']))
isess.run(tf.assign(dr.loss_prior_hyper_weight, loss_weighting['prior_hyper']))
isess.run(tf.assign(dr.loss_Wxx_weight, loss_weighting['prior_Wxx']))
isess.run(tf.assign(dr.loss_Wxxu_weight, loss_weighting['prior_Wxxu']))
isess.run(tf.assign(dr.loss_Wxu_weight, loss_weighting['prior_Wxu']))
...
for epoch in range(MAX_EPOCHS):
    ...
    loss_original = calculate_loss(du_hat, du.get('y'), y_noise, loss_weighting, np.array(du.get('hemodynamic_parameter')))
    ...
    try:
        ...
        loss_updated = calculate_loss(du_hat, du.get('y'), y_noise, loss_weighting, np.array(du.get('hemodynamic_parameter')))
    ...
    y_noise = solve_for_noise_variance(du_hat, du.get('y'), y_noise, loss_weighting)
