...
autoregressor_length = int(8 / t_delta)
autoregressor = np.ones((autoregressor_length, n_stimuli)) / autoregressor_length
e = np.random.randn(n_time_point, n_stimuli) * 0.05
u = copy.deepcopy(e)
for i in range(autoregressor_length, len(e)):
    u[i, :] = np.sum(u[i - autoregressor_length: i, :] * autoregressor, axis=0) + e[i, :]

for s in range(n_stimuli):
    plt.subplot(n_stimuli, 1, s + 1)
    plt.plot(np.array(range(n_time_point)) * t_delta, u[:, s])

for s in range(n_stimuli):
    plt.subplot(n_stimuli, 1, s + 1)
    plt.plot(np.array(range(n_time_point)) * t_delta, abs(dct(u[:, s])))
...
pole = 0.8
order = 1
e = np.random.randn(N) * 0.005

a = [factorial(order) / factorial(i) / factorial(order - i) * np.power(pole, i) * np.power(-1., i + 1) for i in range(1, order + 1)]
...
a = 1
b = 0.1
w = np.array(range(1, N + 1)) * 0.005
w = a * np.power(w, -b)
w = w * (np.random.randn(N) > 0)
v = idct(w)
plt.subplot(2, 1, 1)
plt.plot(w)
plt.subplot(2, 1, 2)
