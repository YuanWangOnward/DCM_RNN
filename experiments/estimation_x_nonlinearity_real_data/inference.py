...
X_NONLINEARITY = 'relu'
EXPERIMENT_PATH = os.path.join(PROJECT_DIR, 'experiments', 'estimation_x_nonlinearity_real_data')
...
SAVE_PATH = os.path.join(EXPERIMENT_PATH, 'results', 'estimation_dcm_rnn_' + X_NONLINEARITY + '.pkl')
SAVE_EXTENDED_PATH = os.path.join(EXPERIMENT_PATH, 'results', 'estimation_dcm_rnn_extended_' + X_NONLINEARITY + '.pkl')
...
A = -np.eye(n_region)
# A[2, 1] = -0.2
B = [np.zeros((n_region, n_region)) for n in range(n_region)]
# B[1][1, 0] = -0.2
...
du_hat._secured_data['u_type'] = 'box_train'
du_hat._secured_data['x_nonlinearity_type'] = X_NONLINEARITY
...
dr.x_nonlinearity_type = X_NONLINEARITY
