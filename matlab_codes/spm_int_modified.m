function [y, states_x, states_h, t] = spm_int_modified(P,M,U)
...
% Integrate
%--------------------------------------------------------------------------
states_x = zeros(v, M.l);
states_h = zeros(v, M.l, 4);
y     = zeros(M.l,v);
J     = M0;
U.u   = full(U.u);
for i = 1:length(t)
    ...
    % collect states
    x_unvectored = spm_unvec(x(2:end),M.x); 
    states_x(i, :) = x_unvectored(:, 1);
    states_h(i, :, :) = x_unvectored(:, 2:end);
